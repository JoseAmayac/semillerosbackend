...
class ProgramControllerUpdateTest extends TestCase
{
    ...
    /** @test */
    public function update_program_without_token_middleware_error()
    {
        $response = $this->putJson('/api/v1/programs/2',[]);
    ...
    /** @test */
    public function update_program_return_request_validation_messages()
    {
        $response = $this->withHeaders([
            'Authorization' => $this->token
        ])->putJson('/api/v1/programs/2',[]);

        $response->assertStatus(422);
        $response->assertJsonStructure(['message','errors']);
        $response->assertJsonValidationErrors(['name','coordinator_id','department_id']);
    }

    /** @test */
    public function update_program_return_invalid_program_id()
    {
        $response = $this->withHeaders([
            'Authorization' => $this->token
        ])->putJson('/api/v1/programs/100',[
            "name" => "Programa modificado",
            "description" => "Descripción del programa modificado",
            "department_id" => 2,
            'coordinator_id' => 2
        ]);
        ...
        $response->assertJsonMissingValidationErrors(["message" => "No query results for model [App\\Models\\Program] 100"]);
    }

    /** @test */
    public function update_program_return_correct_response()
    {
        $response = $this->withHeaders([
            'Authorization' => $this->token
        ])->putJson('/api/v1/programs/2',[
            "name" => "Program modificado 3",
            "description" => "Descripción del programa modificado",
            "department_id" => 3,
            "coordinator_id" => 2
        ]);
        ...
        $response->assertJsonStructure(['program','message']);
        $response->assertJson(['message' => 'Información de programa académico actualizada correctamente']);
    }
}
