...
class DepartmentControllerUpdateTest extends TestCase
{
    ...
    /** @test */
    public function update_department_without_token_middleware_error()
    {
        $response = $this->putJson('/api/v1/departments/1',[]);
    ...
    /** @test */
    public function update_department_return_request_validation_messages()
    {
        $response = $this->withHeaders([
            'Authorization' => $this->token
        ])->putJson('/api/v1/departments/1',[]);
    ...
    /** @test */
    public function update_department_return_invalid_department_id()
    {
        $response = $this->withHeaders([
            'Authorization' => $this->token
        ])->putJson('/api/v1/departments/100',[
            "name" => "departamento modificado",
            "description" => "Descripción del departamento modificado"
        ]);

        $response->assertStatus(404);
        $response->assertJsonStructure(['message']);
        $response->assertJsonMissingValidationErrors(["message" => "No query results for model [App\\Models\\Department] 100"]);
    }

    /** @test */
    public function update_department_return_correct_response()
    {
        $response = $this->withHeaders([
            'Authorization' => $this->token
        ])->putJson('/api/v1/departments/1',[
            "name" => "departamento modificado 1",
            "description" => "Descripción del departamento modificado"
        ]);

        $response->assertStatus(200);
        $response->assertJsonStructure(['department','message']);
        $response->assertJson(['message' => 'Departamento académico actualizado correctamente']);
    }
}
