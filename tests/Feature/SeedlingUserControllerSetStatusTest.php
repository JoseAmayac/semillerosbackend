...
class SeedlingUserControllerSetStatusTest extends TestCase
{
    ...
    /** @test */
    public function update_seedling_user__without_token_middleware_error()
    {
        $response = $this->putJson('/api/v1/changeStatus',[]);
    ...
    /** @test */
    public function update_seedling_user_return_request_validation_messages()
    {
        $response = $this->withHeaders([
            'Authorization' => $this->token
        ])->putJson('/api/v1/changeStatus',[]);

        $response->assertStatus(422);
        $response->assertJsonStructure(['message']);
        $response->assertJsonValidationErrors(['seedling_user', 'status']);
    }

    /** @test */
    public function update_status_return_invalid_seedling_user_id()
    {
        $response = $this->withHeaders([
            'Authorization' => $this->token
        ])->putJson('/api/v1/changeStatus',[
        ...
        $response->assertStatus(422);
        $response->assertJsonStructure(['message']);
        $response->assertJsonValidationErrors(['status', 'seedling_user']);
    }

      /** @test */
      public function update_seedling_user_return_correct_response()
      {
          $response = $this->withHeaders([
              'Authorization' => $this->token
          ])->putJson('/api/v1/changeStatus',[
          ...
          $response->assertJsonStructure(['seedling_user','message']);
          $response->assertJson(['message' => 'Usuario inscrito al semillero con éxito']);
      }
